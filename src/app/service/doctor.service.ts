...
export class DoctorService {
    ...
    getIdDoctorByUser(): Observable<User> {
        const url = `${this.urlBasic}user/find_by_username`;
    ...
    getDoctorById(id: number): Observable<Doctor> {
        const url = `${this.urlBasic}doctor/find_by_id/${id}`;
        return this.http.get(url)
            .pipe(map(this.extractData));
    }

    updateAddress(id: number, address: Address): void {
        const url = `${this.urlBasic}doctor/update_address/${id}`;
    ...
    getAll(): Observable<Doctor[]> {
        const url = `${this.urlBasic}doctor/find_all`;
        return this.http.get(url)
            .pipe(map(this.extractData));
    }

    createDoctor(doctor: Doctor): void {
        const url = `${this.urlBasic}user/doctor/create`;
        const header: HttpHeaders = new HttpHeaders({'Content-Type': 'application/json'});
